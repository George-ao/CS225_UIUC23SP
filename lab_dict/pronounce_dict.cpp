...
{
    /* Your code goes here! */
    string word1_ = word1;  std::transform(word1_.begin(), word1_.end(), word1_.begin(), ::toupper); //std::cout<<word1_<<dict.find(word1_)<<std::endl;
    string word2_ = word2;  std::transform(word2_.begin(), word2_.end(), word2_.begin(), ::toupper);
    if(dict.find(word1_)!=dict.end()&&dict.find(word2_)!=dict.end())
    {
        if(dict.find(word1_)->second==dict.find(word2_)->second)
            return true;
    }
    return false;
}
