...
vector<Point<3>> v1;
for (int i = 0; i < 3; i++){
v.push_back(Point<3>(i,i));
 v1.push_back(Point<3>(2*i,2*i));
}
 KDTree<3> tree3 = KDTree<3>(v);
 KDTree<3> newtree = KDTree<3>(v1);
  // Rnewtree == tree );
 newtree = tree3;
 newtree = tree2;

  // tree2 = tree;
  REQUIRE( newtree.findNearestNeighbor(target) == expected );
  REQUIRE( tree2.findNearestNeighbor(target) == expected );
  REQUIRE( tree.findNearestNeighbor(target) == expected );
}
// TEST_CASE("my", "[weight=1][part=1]") {
//   double coords[3][3] = {{0, 0, 100},   // mine
//                           {50, 50, 0},
//                           {0, 100, 0}  // mine
//  };
//   bool isMine[3] = {0, 0,  0};
