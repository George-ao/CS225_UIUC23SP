...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    //legnth not equal, impossible
    if(word.length()!=test.length()) return false;

    Dict<char, int> hashTable_1(256);
    for (size_t i = 0; i < test.length(); i++)
        hashTable_1[test[i]]++;
    Dict<char, int> hashTable_2(256);
    for (size_t i = 0; i < word.length(); i++)
        hashTable_2[word[i]]++;

    vector<pair<char, int>> ret;
    vector<pair<char, int>> ret2;

    typename Dict<char, int>::iterator it;
    for (it = hashTable_1.begin(); it != hashTable_1.end(); it++) {
            if(it->second!=hashTable_2.find(it->first))
            {
                return false;
            }
    }
