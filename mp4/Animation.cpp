...
void Animation::write(const std::string& filename) {
    ...
    // system(("ffmpeg -i frames/" + name + "%02d.png -vcodec libx264 -r 30 -pix_fmt yuv420p -vf \"scale=trunc(iw/2)*2:trunc(ih/2)*2\" " + filename + ".mp4").c_str());
system(("ffmpeg -i frames/" + name + "%0" + to_string(to_string(frames.size()).length()) + "d.png -vcodec libx264 -r 30 -pix_fmt yuv420p -vf \"scale=trunc(iw/2)*2:trunc(ih/2)*2\" " + filename + ".mp4").c_str());
}
