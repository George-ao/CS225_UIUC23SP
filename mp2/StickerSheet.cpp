...
namespace cs225
{
    ...
    {
        //3.7 new 
        //std::cout<<cur_counter<<"   "<<arr_couter<<" "<< arr_pointer[0]<<std::endl;
        if(cur_counter==arr_couter) 
        {
            for(unsigned int add_counter=0;add_counter<cur_counter;add_counter++)
            {
                ...
                if(add_counter==cur_counter-1) return -1;
    ...
    bool StickerSheet::translate (unsigned index, unsigned x, unsigned y)
    {
        ...
        if(arr_pointer[index]==NULL) return false;
