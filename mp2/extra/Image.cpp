...
namespace cs225
{
    ...
    void Image::scale(double factor)
    {
        ...
        for(unsigned x = 0; x < new_width; x++)
        {
            ...
            {        
                HSLAPixel *pixel_1 = this->getPixel(floor(x/factor),floor(y/factor));          //original
    ...
    void Image::scale(unsigned w, unsigned h)
    {
        ...
        double factor2 = h/height;
        for(unsigned x = 0; x < w; x++)
        {
            for(unsigned y = 0; y < h; y++)
            {        
                HSLAPixel *pixel_1 = this->getPixel(floor(x/factor1),floor(y/factor2));          //original
                HSLAPixel *pixel_2 = new_image->getPixel(x,y);          //new  
                *pixel_2=*pixel_1;
            }
        }
