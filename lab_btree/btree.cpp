...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if((first_larger_idx<=subroot->elements.size()-1) && !subroot->elements.empty()) 
    {
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    //3rd
        //copy elem to the right into new right
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if((first_larger_idx<=subroot->elements.size()-1) && !subroot->elements.empty()) 
    {
    if(subroot->elements[first_larger_idx]==pair.value)
        return;
    }
    //if not leaf, recurse down
    if(!subroot->is_leaf)
    {
        //recurse down
        insert(subroot->children[first_larger_idx],pair);
        //check out of order when back to top
