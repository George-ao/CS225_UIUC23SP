...
class BinaryTree
{
  ...
  private:
     ...
     * @param subRoot The current node in the recursion
     */  
    //void isOrdered(Node* subroot, bool& return_val, T& prev) const;
    void isOrdered(Node* subroot, vector<T> &v_) const;
    /**
     * Private helper function for the public printPaths function.
     * @param paths The current node in the recursion
     */  
    void printPaths(vector<vector<T> > &paths,vector<T> single_paths,Node* subroot) const;
    /**
     * Private helper function for the public sumDistances function.
     * @param paths The current node in the recursion
     */  
    void sumDistances(Node*subroot, int sum,vector<T> &store_sum,int& count) const;
    /**
