...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    //2nd
        //
    // while(singleQueue.size()!=0 || mergeQueue.size()!=0)
    while(singleQueue.size()+mergeQueue.size()>1)
    {

    TreeNode* left_ =  removeSmallest(singleQueue,mergeQueue);
    TreeNode* right_ =  removeSmallest(singleQueue,mergeQueue);
    TreeNode* merge_node = new TreeNode(left_->freq.getFrequency()+right_->freq.getFrequency());
    merge_node->left=left_;
    merge_node->right=right_;
    mergeQueue.push(merge_node);

    }
    //if only one left
    if(singleQueue.size()==0 && mergeQueue.size()==1)
    {
        root_=mergeQueue.front();return;
    }
    if(singleQueue.size()==1 && mergeQueue.size()==0)
    {
        root_=singleQueue.front();return;
    }
}
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        bool cur_bit = bfile.getNextBit();
            //left
        if(cur_bit==false) current = current->left;
            //right
        if(cur_bit==true) current = current->right;
        // check leaf ?
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
    //TreeNode* current = root_;
    if(bfile.hasBits()==false) return NULL;
    else
    {
        bool get_bit_ = bfile.getNextBit();
        //leaf
        if(get_bit_) 
        {
            Frequency freq_(bfile.getNextByte(),0);
            TreeNode* leaf_node_ = new TreeNode(freq_);
            return leaf_node_;
        }
        else
        {
            Frequency freq_(0);
            TreeNode* internal_node_ = new TreeNode(freq_);       
            internal_node_->left = readTree(bfile);
            internal_node_->right = readTree(bfile);
